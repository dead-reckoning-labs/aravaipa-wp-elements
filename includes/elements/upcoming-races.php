...
// race page URL, image URL, ISO end date, live/results URL. Named because the
// row parser counts backwards from it, so the two have to agree.
if ( ! defined( 'ARV_RACES_COLUMNS' ) ) {
	define( 'ARV_RACES_COLUMNS', 11 );
}
		...
		'values'  => cs_compose_values(
			array(
				...
				'rows'      => cs_value(
					...
					"Mogollon Monster Trail Runs | 2026-09-12 | September 12-13 | 100 Mile, 42K | Mogollon Rim | Pine, AZ | https://ultrasignup.com/register.aspx?did=130408 | https://www.aravaiparunning.com/mogollon-monster/ | https://i0.wp.com/www.aravaiparunning.com/avr/wp-content/uploads/2019/06/Mogollon-Monster-Run-logo.png?w=810&ssl=1 | 2026-09-13 | \n" .
 ...
 *
 * A row can carry its own live URL (a broadcast, a tracker) in the last
 * column; without one, both live and results fall back to UltraSignup.
 *
 * @param array  $race
 * @param string $today Y-m-d in site time.
...
function arv_upcoming_races_parse_row( $row ) {
	...
	if ( $count >= ARV_RACES_COLUMNS ) {
		$tail      = array_slice( $row, $count - 7 );
		$distances = implode( ' | ', array_slice( $row, 3, $count - 10 ) );
		return array(
			'name'      => $name,
			'iso'       => $iso,
