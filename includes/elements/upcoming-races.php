...
// race page URL, image URL, ISO end date. Named because the row parser counts
// backwards from it, so the two have to agree.
if ( ! defined( 'ARV_RACES_COLUMNS' ) ) {
	define( 'ARV_RACES_COLUMNS', 10 );
}
...
function arv_upcoming_races_parse_row( $row ) {
	...
	if ( $count >= ARV_RACES_COLUMNS ) {
		$tail      = array_slice( $row, $count - 6 );
		$distances = implode( ' | ', array_slice( $row, 3, $count - 9 ) );
		return array(
			'name'      => $name,
			'iso'       => $iso,
